...
class MAP_vid(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : '/home/bennyzhu/ILISTO/logs/shots/pretrain/DAUB_400/best/', 
    ...
    def generate(self, onnx=False):
        self.net    = Model(self.num_classes, num_frame=4)
        device      = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.net.backbone.load_state_dict(torch.load(self.model_path + 'backbone.pth', map_location=device))
        self.net.neck.load_state_dict(torch.load(self.model_path + 'neck.pth', map_location=device))
        self.net.head.load_state_dict(torch.load(self.model_path + 'head.pth', map_location=device))
        #self.net.loss_p.load_state_dict(torch.load(self.model_path + 'loss_p.pth', map_location=device))
