...
class LossHistory():
    ...
    def append_loss(self, epoch, loss, val_loss,loss_s_var):
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)

        self.losses.append(loss)
        self.val_loss.append(val_loss)
        self.loss_s_var.append(loss_s_var)
        ...
        with open(os.path.join(self.log_dir, "epoch_val_loss.txt"), 'a') as f:
            ...
            f.write("\n")
        with open(os.path.join(self.log_dir, "epoch_loss_s_var.txt"), 'a') as f:
            f.write(str(loss_s_var))
            f.write("\n")
        self.writer.add_scalar('loss', loss, epoch)
        self.writer.add_scalar('val_loss', val_loss, epoch)
        self.writer.add_scalar('loss_s_var', loss_s_var, epoch)
        self.loss_plot()

    def append_loss_orin(self, epoch, loss, val_loss):
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)
...
class EvalCallback():
    ...
    def get_history_imgs(self, line):
        ...
        #return [os.path.join(dir_path,  "%s.%s" % ("%06d" % max(id, 1),file_type)) for id in range(index - 4, index + 1)]
        return [os.path.join(dir_path,  "%s.%s" % (max(id, 0),file_type)) for id in range(index - 3, index + 1)]
